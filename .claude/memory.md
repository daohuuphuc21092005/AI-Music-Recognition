...
- **Hạng mục đã hoàn thành**:
  ...
  - [x] Bộ test tự động: **242 PASS / 2 SKIP** (2026-09-17, sau §4).
...
- **Hạng mục tiếp theo (Next Actions)**:
  ...
  - [ ] **Độ trễ Tầng 1: 4,8 s/truy vấn** với 24.375 fingerprint — chiếm ~82% độ trễ end-to-end của EXP-08 (5,8 s). Đối chứng lọc theo hash trên đủ 1.900 truy vấn đã xong (0 lệch quyết định, xem nhật ký 2026-09-17); chưa viết mã.
  - [ ] **Tầng Cover đọc trọn truy vấn** (thay 30 s đầu): tempo chậm 27% → 100% @1 trên truy vấn có trong CSDL (`experiments/exp07_cover/query_span_check.py`). Phải chạy lại EXP-07 (nhiễu + held-out) trước khi đổi server.
  - [ ] **§13 Robust retrieval**: MERT Recall@5 0.7921 < 0.80 (thiếu do dịch cao độ); MERT∪Cover 0.9926. Chờ chủ dự án quyết có điều chỉnh tiêu chí không.
  - [ ] (tuỳ chọn) Chạy lại EXP-01/EXP-07 dưới `HeldOutProtocol` để FPR/FMR hết là cận trên.
