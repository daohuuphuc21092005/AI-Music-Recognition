...
- **Hạng mục đã hoàn thành**:
  ...
  - [x] Tầng 1 (Chromaprint): Codec thuần Python, vectorised bit-error matching, **τFP = 0.30** (`EXP-01` trên 24.375 fingerprint thật / 1.900 truy vấn). Lọc ứng viên theo hash (`FP_PREFILTER_*`, 2026-09-17): ~110 ms/truy vấn thay vì ~4,5 s, 0 lệch quyết định trên 1.900 truy vấn.
  - [x] Tầng 2 (MERT Deep Retrieval): MERT-v1-95M + FAISS FlatIP, **τMERT = 0.98** (`EXP-06` FMR 2,65% trên 48.750 vector; `EXP-02`/`EXP-03` chạy lại 2026-09-17).
  - [x] Tầng 3 (Cover Service): CQT + Optimal Transposition Index, cắt truy vấn theo hệ số nhịp độ 0.90–1.10 (`COVER_TEMPO_FACTORS`), **τCover = 0.90** (`EXP-07` hiệu chỉnh lại 2026-09-17: P 0.9952 / R 0.7663 / FMR 0,11%) — **đã nối chính thức vào `cascade_service.py`** (nhánh `STAGE_3_COVER`, bật qua `config.COVER_ENABLED`).
  ...
  - [x] Bộ test tự động: **252 PASS / 2 SKIP** (2026-09-17, sau lọc hash Tầng 1 + Cover nhiều độ dài cắt).
...
- **Quy mô artifact sau đợt nạp FMA medium (2026-09-16)**:
  ...
  | data/test_queries/manifest.csv | 2.090 | 100 bài nguồn × 19 biến đổi + 10 bài CC0 × 19 (ghi nối 2026-09-17 cho lớp LOW của EXP-08; `query_span_check.py` và EXP-04 `--sources` lấy 100 bài đầu) |
...
- **Hạng mục tiếp theo (Next Actions)**:
  ...
  - [ ] (tuỳ chọn) Chạy lại EXP-01 dưới `HeldOutProtocol` để FPR hết là cận trên (EXP-07 đã chạy lại).
  - [ ] Lỗi 422 (RequestValidationError) chưa theo khuôn lỗi chuẩn `{"detail": {"error_code", "message"}}` — cần exception handler riêng + test trong `tests/test_api.py`.
...
- **2026-09-16 (hiệu chỉnh lại ngưỡng trên corpus 24.375 bài có audio)**:
  ...
  - **Kế hoạch "kết quả đúng + đo lại được" (2026-09-16)** — §1 dữ liệu quyền trung thực (commit `8e66544`): 100k bản ghi dataset_G gắn nguồn `SIMULATED`, `verified_at` rỗng; khoản phạt mới `simulated_unverified 0.10` đưa SIMULATED chưa xác minh xuống 0.40 < 0.50 nên `rights_gate` chặn. §2 backend mã lỗi: lưới ngoại lệ toàn cục `INTERNAL_ERROR`, kiểm UUID trước CSDL, `extract_mert_embedding` ném `EmbeddingAudioError`/`EmbeddingModelError` thay vì trả `None` (lỗi model trước đây hiện ra là `NO_AUDIO`), `/health` và evidence Cover không còn đường dẫn máy chủ, bỏ mật khẩu CSDL mặc định khỏi mã nguồn, giao diện chỉ nhận `source_url` http(s) và hiện nhãn "SUY ĐOÁN" khi `rights.predicted`. Bộ test **219 pass / 5 skip**. Còn §3 (hạ tầng thí nghiệm: lỗ rò held-out EXP-08, EXP-02 tràn RAM, EXP-04 thiếu tầng Cover) và §4 (chạy lại ở 24.375 bài, điền bảng §13).
  - **Đối chứng Tầng 1 v2 — đủ 1.900 truy vấn (2026-09-17)**, `scratchpad/compare_fp_prefilter_v2.py`, TOP_K 50/200/1000: 0 lệch quyết định; **trong dải sát ngưỡng τFP ± 0.15 (30 truy vấn, 1,6%) điểm đường lọc TRÙNG KHÍT điểm chấm đầy đủ (chênh lớn nhất 0.0000)** ở cả ba mức TOP_K; không truy vấn được chấp nhận nào có chênh > 0. Chênh 0.1086 của lần trước chỉ nằm ở truy vấn có điểm đầy đủ ≤ 0.113 (audio_overlay, đổi tốc độ — 0 hash trùng), xa dưới τFP. TOP_K = 50 là đủ. Nghĩa là thiết kế "lọc theo hash + quay về chấm đầy đủ khi ứng viên tốt nhất nằm trong ±0.15" an toàn trên dữ liệu này. Vẫn là bằng chứng THỰC NGHIỆM, không phải cận chứng minh được: trường hợp điểm lọc < 0.15 mà điểm đầy đủ ≥ 0.30 cần chênh ≥ 0.15, lớn nhất quan sát được là 0.1086. Chưa viết mã — ngoài phạm vi kế hoạch hiện tại. (Lượt chạy bị ngắt ở 1.701/1.900 khi máy khởi động lại lúc 12:42, script tự chạy tiếp từ CSV.)
  - **§3 hạ tầng thí nghiệm (2026-09-17)**: `exclude_recording_ids` đi xuyên cả ba tầng của `cascade_service` (thay `HeldOutSession`/`filtered_index` của EXP-08); `experiments.common.HeldOutProtocol` loại bản trùng, bản gần trùng (Chromaprint nguyên clip ≥ 0.10) và bài bị trộn chồng. Phát hiện: `audio_overlay` trộn chồng bài nguồn KẾ TIẾP trong manifest — bài đó vẫn nằm trong CSDL, nên 2/4 "nhận nhầm" held-out của EXP-01 ở τFP 0.30 thực ra là nhận ĐÚNG; và "Digg It! (12\" FunkMixx)" / "(7\" FunkEdit)" là bản gần trùng (đoạn cắt 10–15 s khớp 0.35). EXP-02 tính theo khối; EXP-04 có đủ 5 hệ (thêm Cover và cascade production) — chạy thử 1 bài: Cover cứu 4/4 truy vấn dịch cao độ và tempo 1.05/1.10; EXP-05 từ chối ghép khi EXP-01 và EXP-04 lệch bộ truy vấn (đang lệch 0/1.900); `run_all_experiments` chạy hiệu chỉnh → ghi ngưỡng → kiểm ngưỡng runtime khớp `rules_v1.yaml` → tiêu thụ.
  - **CSDL thật chạy trong Docker, cổng 5433** (`.env`), không phải PostgreSQL cài sẵn ở 5432. Máy khởi động lại thì phải bật Docker Desktop trước, nếu không 24 test CSDL bị skip và thí nghiệm chết ngay lúc kết nối.
  - **§4 xong (2026-09-17)** — chuỗi `scratchpad/run_section4.py` tách nền: EXP-02 → 03 → 04 → 05 → sinh truy vấn CC0 → EXP-08 `--per-license 3`. **EXP-02** (48.750 vector, tính theo khối, 3,4 phút): Recall@1 0.8787 · Recall@5 0.9374 · MRR 0.9059 · mAP 0.9036; 1.196 cặp KHÁC bản ghi (trên 1,19 tỉ) vẫn ≥ τMERT 0.98. **EXP-03** (GPU, 200 cửa sổ reference / 6.059 cửa sổ truy vấn, 53,7 phút): mean R@1 0.8421 / R@5 0.9264; mean+std R@1 0.8534 / R@5 0.9335 → chỉ +1,13pp mà index ×2, **giữ mean**. **EXP-04** (1.900 truy vấn, 197 phút): cascade production FP→MERT→Cover P 0.9967 / **R 0.8047** / F1 0.8905 (FP→MERT chỉ R 0.5726); cả 5 nhận sai đều là `audio_overlay` nhận ra bài bị trộn chồng (có trong CSDL). **Recall@5 truy vấn biến đổi trên toàn chỉ mục: MERT 0.7921 — thiếu 0.008 so với §13**, toàn bộ do dịch cao độ (MERT@5 0.185; bỏ pitch 0.954); Cover 0.8542; MERT∪Cover 0.9926. **EXP-05**: dịch cao độ 0,25% → 71% nhờ Cover; đổi tốc độ 45% là điểm yếu lớn nhất còn lại (bản chậm 8–10%). Sửa dòng in "Cả hai tầng" của EXP-05 (in nhầm số cascade CÓ Cover). EXP-03 từng chết ngay trên GPU vì input MERT để ở CPU (sửa ở `5418a35`). **Điểm yếu tầng Cover đã tìm ra nguyên nhân** (`experiments/exp07_cover/query_span_check.py`, 100 bài nguồn): server lấy 30 s đầu truy vấn rồi co về 64 khung, nên bản chậm bị cắt mất phần cuối — tempo 0.90 đúng @1 27% / qua τ 1% với 30 s đầu, nhưng **100% / 83%** khi đọc trọn truy vấn; tempo 0.95: 64%/2% → 100%/77%; tempo nhanh không đổi. Đoạn cắt 10/15 s so với reference cùng khoảng đạt 1.000 (lệch trục thời gian thuần), nhưng nhóm này Chromaprint đã bắt 100%. Chưa đổi server: mới đo trên truy vấn CÓ trong CSDL, phải chạy lại EXP-07 (nhiễu + held-out) để biết FMR trước. Tác vụ chờ nền của Claude Code liên tục bị cắt vì RAM thấp (~2,3 GB trống) — chuỗi tách bằng `Start-Process` không bị ảnh hưởng.
  - **§4 kết quả EXP-08 & nghiệm thu §13 (2026-09-17)**: EXP-08 `--per-license 3` (20 bài nguồn, 380 truy vấn × 2 điều kiện × 2 ngữ cảnh = 1.520 lượt, 76 phút): **Macro-F1 0.9091**, đủ 4 lớp (LOW 114 nhờ CC0), held-out 760/760 ra UNKNOWN (FMR 0,00%, không lỗ rò), nhận đúng bài có trong CSDL 80,3%. 144/150 lần nhận diện thất bại ra UNKNOWN; 70/150 là tempo chậm. Lỗi rủi ro duy nhất không phải UNKNOWN: `audio_overlay` nhận ra bài bị trộn chồng (NC) → HIGH thay vì CONDITIONAL — cả 6 lần nhận bản ghi khác đều là bài trộn chồng; giới hạn thật là pipeline chỉ báo MỘT bài mỗi truy vấn. Bảng §13: clean P=R=1.0 ✅ · Robust Recall@5 MERT 0.7921 ❌ (Cover 0.8542, MERT∪Cover 0.9926) · FMR ✅ · Macro-F1 ✅. README (bảng trạng thái, §13, EXP-04/05/07/08, mục giới hạn dữ liệu — 158.117 bản ghi / 24.375 có audio) và `rules/06` §5 đã cập nhật.
  - **Làm "tất cả" 4 việc sau §4 (2026-09-17 tối)** — (1) **Tầng 1 lọc hash** (`6676f70`): chỉ chấm 50 bản ghi nhiều hash trùng nhất, quay về quét toàn bộ khi điểm trong ±0.15 quanh ngưỡng hoặc truy vấn < 10 s; kiểm hàm production trên 1.900 truy vấn so với điểm quét đầy đủ đã lưu: 0 lệch quyết định, 30 lần (1,6%) quét toàn bộ, TB 110 ms / P50 37 ms. (2) **Cover cắt theo hệ số nhịp độ** (`f8b1de2`, sửa nhãn `2bf97b2`): descriptor cho từng độ dài 30/f, lấy max; lỗi phát hiện khi chạy EXP-07 — các dòng trùng (file ngắn hơn độ dài cắt) làm hệ số 0.90 luôn thắng hoà nên evidence ghi "chậm 0.90×" cho cả bản gốc; nay mỗi độ dài thật một dòng, gắn hệ số gần nhất (±0,5 s) hoặc None. `query_span_check --off-grid`: tempo 0.92 (ngoài bảng hệ số) @1 43% → 100%, qua τ 2% → 82% — không chỉ khớp lưới của tập kiểm thử. EXP-07 (`--sources 100`, manifest đã ghi nối CC0): τCover vẫn 0.90, R 0.6837 → 0.7663, FMR 0,11%, nhiễu max 0.7347, @1 88,8%. EXP-04 (`--sources 100`, 57 phút): cascade R 0.8805 / F1 0.9352 (trước 0.8047/0.8905), TB 906 ms (trước 5.411 ms), đổi nhịp 45% → 81%; 5 nhận sai vẫn đều là bài trộn chồng. EXP-08: **Macro-F1 0.9518** (trước 0.9091), nhận đúng known 87,9%, thất bại vì tempo chậm 70 → 12, TB 1,4 s; dừng một lần ở 338/380 vì `CUDA error: an illegal memory access` (backend trả đúng MODEL_FAILURE), chạy tiếp từ checkpoint. (3) **§13 Robust retrieval chấm cấp hệ thống** theo quyết định chủ dự án: `system_recall@5` = MERT ∪ Cover = **0.9937** ✅, MERT đơn lẻ 0.7921 luôn báo kèm; biên bản lý do ở `rules/06` §5. §13 đạt cả 4. (4) **Docker M8**: `.dockerignore` (chặn `.env*`, `data/`), non-root uid 10001, cổng chỉ 127.0.0.1, `POSTGRES_PASSWORD` bắt buộc (bỏ mật khẩu mặc định viết sẵn ở compose và `.env.example`; ⚠️ mật khẩu CSDL đang dùng TRÙNG đúng chuỗi mặc định đó — công khai trong lịch sử git, nên đổi; CSDL chỉ mở 127.0.0.1), mount `AUDIO_ROOT` → `/audio` chỉ đọc, GPU qua `deploy.resources`, healthcheck đọc `status == ONLINE` (vì `/health` luôn 200), ghim `MERT_MODEL_REVISION=12af15f…` (trust_remote_code). **Compose đặt `name: music-recognition-main`**: CSDL thật là container `music_rights_db` của project cũ (volume `music-recognition-main_postgres_data`) — tên project mặc định theo thư mục sẽ tạo CSDL rỗng mới. Chủ dự án chọn build GPU. **Build lần đầu thất bại vì `requirements.txt` sai từ commit đầu**: ghim `transformers==4.57.6` (đòi huggingface-hub<1.0) cùng `huggingface_hub==1.28.0` -> pip ResolutionImpossible, tức cài mới từ file này chưa bao giờ chạy được; môi trường thật dùng transformers 5.15.1 / tokenizers 0.22.2 (MERT chạy bình thường) — nay ghim đúng 11 gói theo môi trường đã kiểm chứng. Kiểm chứng image (9,7 GB): không có `.env`/`data/`/`.git`, user `app`, mã chỉ đọc, CUDA thấy GTX 1650; `/health` ONLINE qua compose; `POST /api/v1/search` tempo 0.90 -> COVER_MATCH đúng bài, tempo_factor 0.9, 2,4 s (lần đầu ~2 phút do tải MERT đúng revision ghim). Container backend đã `stop` sau khi kiểm. **Ổ C còn ~9,4 GB** sau build (build cache Docker cũ 13,4 GB của dự án khác chưa dọn). Container CSDL cũ `restart: always` -> lần `docker compose up` đầy đủ đầu tiên sẽ tạo lại nó (volume giữ nguyên). Phát hiện chưa sửa: lỗi kiểm tra đầu vào (422, vd. `platform=youtube` thay vì `YOUTUBE`) trả khuôn mặc định FastAPI (danh sách) chứ không phải `{"detail": {"error_code", "message"}}` của rules/05.
