...
- **`EXP-07` — Cover Identification** *(Nếu triển khai AI-3)*:
  ...
  - **Quy mô reference đổi thì phải chạy lại**: cùng bộ truy vấn, τ = 0.70 cho FMR 5,2% khi reference chỉ có 100 bài nguồn (giao thức cấp cửa sổ) nhưng 18,6% trên chỉ mục 24.375 bài (điều kiện server) — `experiments/results/exp07_cover.json`; lượt trước khi đổi cách cắt truy vấn đo 17,4% (commit `b948285`).
  - **EXP-07b — luật khoảng cách hạng 1 – hạng 2** (`experiments/exp07_cover/margin_rule.py --sources 100` → `exp07_cover_margin_rule.json`; chỉ ĐO, production chưa đổi — quyết định của chủ dự án 2026-09-18): cùng điều kiện server với EXP-07, quét `s1 ≥ τ_abs HOẶC (s1 ≥ τ_low VÀ s1 − s2 ≥ δ)`, chọn theo nhiễu 0 → FMR ≤ 0,5% → recall, **kiểm tra chéo 2 nửa theo bài nguồn** (số đọc chính). Baseline tái lập đúng EXP-07 (P 0.9952 / R 0.7663 / FMR 0,11%). Kiểm tra chéo: R 0.7432 → 0.8779 (FMR 0 → 0) và 0.7895 → 0.8863 (FMR 0,21% → 0,53%). Luật `s1 ≥ 0.90 | (s1 ≥ 0.70 & s1 − s2 ≥ 0.08)` trên cả 1.900 truy vấn: R 0.8821, FMR vẫn đúng 2 ca đoạn cắt ngắn của baseline, dịch cao độ 71% → 99,5%, đổi nhịp 79,5% → 100% — δ 0.08 chọn SAU KHI thấy số. Khoảng cách trung vị: bài có trong CSDL dưới τ 0.315, bài lạ 0.009. `rule_comparison.*.overlay_wrong_is_mixed_in_song` đếm các ca chồng âm nhận ra bài bị trộn (14/14 ở luật trên). Bật luật thì phải chạy lại EXP-04/05/08.
...
| **Unknown Detection** (Từ chối nhận diện) | False Match Rate (FMR) | **FMR ≤ 5%** | ✅ FP 0,00% (τFP 0.30) · MERT 2,65% · Cover 0,11% · cả pipeline 0,00% (EXP-08, 760 lượt held-out) — cả ba tầng đo theo `HeldOutProtocol` |
...
- **Biên bản giữ τFP = 0.30 (2026-09-18, chủ dự án quyết định)**: EXP-01 đo lại dưới `HeldOutProtocol` (1.900 truy vấn, `--sources 100`) cho FPR held-out 0.37% / 0.11% / 0.0 ở τ 0.10 / 0.15 / ≥0.20 (giao thức cũ: 2,26% / 1,63% / 0,68%), nên quy tắc "F1 tầng 1 cao nhất trong nhóm FPR ≤ 0.005" nay ra **0.10**. Giữ 0.30 vì đo ở cấp hệ thống (mô phỏng từ EXP-01 + EXP-04, tái lập đúng EXP-04 ở 0.30): F1 cascade chỉ 0.9352 → 0.9368, nhận sai 5 → 8, 1 lần nhận nhầm bài lạ, và bộ lọc hash phải quét toàn bộ 44% truy vấn (1,6% ở 0.30) — Tầng 1 từ ~0,11 s lên ~2 s. Nhiễu trắng 30 s đạt 0.0814 trên 24.375 bài: lề ở 0.10 chỉ 0.019. `apply_calibrated_thresholds.py` nay KHÔNG tự hạ ngưỡng (cần `--allow-loosen`), nên `run_all_experiments.py` không lật lại quyết định này.
