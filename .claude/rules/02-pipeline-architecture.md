...
- **Luật chấp nhận hiện tại là điểm tuyệt đối `s1 ≥ τCover`.** EXP-07b (`experiments/exp07_cover/margin_rule.py`) đo được Cover xếp đúng hạng 1 cho mọi truy vấn dịch cao độ / đổi nhịp nhưng loại ~25% vì điểm < 0.90, và luật thêm nhánh khoảng cách `(s1 ≥ 0.70 VÀ s1 − s2 ≥ 0.08)` nâng recall 0.766 → 0.882 mà không thêm lượt nhận nhầm held-out nào (kiểm tra chéo: 0.74/0.79 → 0.88/0.89). **Chưa bật — đổi luật chấp nhận là quyết định của người**, như hạ τFP; bật thì `cover_service` phải trả thêm điểm hạng 2 (bản ghi khác), cập nhật `identity_gate` trong `rules_v1.yaml` nếu cần và chạy lại EXP-04/05/08.
